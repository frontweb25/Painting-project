...
__webpack_require__.r(__webpack_exports__);
const drop = () => {
  const fleInputs = document.querySelectorAll('[name="upload"]');
  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(eventName => {
    fleInputs.forEach(input => {
...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      e.preventDefault(); //Добавление родителя в форму, для оповещения
      ...
      setTimeout(() => {
        //скрытие формы
        item.style.display = 'none';
      }, 400); //Добавление картинки состояния зопроса в форму

      let statusImg = document.createElement('img');
      statusImg.setAttribute('src', message.spinner);
      statusImg.classList.add('animated', 'fadeInUp');
      statusMessage.appendChild(statusImg); // Оповещение пользователя о состоянии запроса
      ...
      postData(api, formData).then(res => {
        ...
        textMessage.textContent = message.success; //присваивание текста из обьекта
      }).catch(() => {
        statusImg.setAttribute('src', message.fail);
        textMessage.textContent = message.failure; //присваивание текста из обьектаЫ
      }).finally(() => {
        clearInputs();
        setTimeout(() => {
...
const modals = () => {
  ...
  function showModalByScroll(selector) {
    window.addEventListener('scroll', () => {
      ...
      if (!btnRessed && window.pageYOffset + document.documentElement.clientHeight >= scrollHeigth) {
        document.querySelector(selector).click();
      }
    });
  }

  function windowHideModal(elem) {
    elem.forEach(item => {
      item.style.display = 'none';
      item.classList.add('animated', 'fadeIn');
    });
  ...
  showModalByScroll('.fixed-gift'); //  showModalByTime('.popup-consultation', 3000);
};
...
const sliders = (slides, dir, prev, next) => {
  ...
  function showSlides(n) {
    ...
    items[slideIndex - 1].style.display = 'block'; //при рендере страницы выводим 1 слайд
  }
